...
// ---------------------------------------------------------------------------
// BlockingQueue (阻塞队列 / 教科书标准固定容量环形实现)
//
...
//
// 阻塞语义：当队列满时，生产者入队操作会被"阻塞"——本演示以提示帧模拟
//           "阻塞等待中"，直到消费者出队腾出空间后才能继续入队。
//           （线程安全实现中即生产者线程 park，直至 notify）
//
// 布局：环形排列（类似旋转寿司传送带），槽位按索引排成圆环，
//       有值的显示数据，空槽显示虚线框。front 标记 F，rear 标记 R。
// ---------------------------------------------------------------------------
class BlockingQueue : public IDataStructure {
public:
    ...
    DSKind kind() const override { return DSKind::BlockingQueue; }
    std::string name() const override { return "阻塞队列 BlockingQueue"; }
    std::string description() const override {
        return "固定容量环形缓冲区，front/rear 指针取模运算，牺牲1单元判空满。"
               "满时阻塞等待（本演示以提示模拟），出队腾位后继续。常用于生产者-消费者模式。";
    }
    ...
    FrameList enqueue(const std::string& value) {
        ...
        if (full()) {
            // 阻塞语义：满时无法入队，模拟"阻塞等待"
            frames.push_back(makeFrame("队列已满！阻塞等待中（消费者出队后方可继续入队）",
                {}, "#FF5722"));
            return frames;
        }
    ...
    FrameList clear() override {
        FrameList frames;
        frames.push_back(makeFrame("清空阻塞队列", {}, "#FF9800"));
        for (int i = 0; i < Capacity; ++i) {
            slots_[i].value = "";
