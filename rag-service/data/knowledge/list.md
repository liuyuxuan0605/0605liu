...
本文系统讲解**链表（Linked List）**这一线性数据结构，以及它的变体：栈（Stack）、队列（Queue）、双向链表（Doubly Linked List / DLL）、双端队列（Deque）。
链表的核心特征是节点在内存中**非连续存储**，通过指针（引用）串联；这正是它与**数组（Array）**最本质的区别——数组在内存中连续存储、支持 O(1) 随机访问，而链表随机访问需要 O(N) 从头遍历。

## 链表与数组的区别（Linked List vs Array 对比）

面试常问「链表和数组有什么区别」，核心差异如下表：

| 维度 | 链表（Linked List） | 数组（Array） |
| --- | --- | --- |
| 内存布局 | 节点**非连续**存储，靠指针串联 | 元素**连续**存储在一块内存 |
| 随机访问 get(i) | O(N)，必须从头遍历 | O(1)，直接下标访问 |
| 头部插入 / 删除 | O(1) | O(N)，需搬移后续元素 |
| 尾部插入（已知尾指针） | O(1) | O(1)（动态数组均摊 O(1)） |
| 尾部删除 | 单链表 O(N)、双链表 O(1) | O(1) |
| 空间开销 | 每节点额外指针（双向链表多一个 prev 指针） | 可能预留空间浪费，满时需扩容搬移（O(N)） |
| 适用场景 | 频繁在头尾增删、大小不确定（如实现栈 / 队列） | 频繁随机访问、大小较固定 |
