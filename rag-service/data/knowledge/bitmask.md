...
> **中文导读（面向中文提问）**：位运算 / 位操作（Bitmask / Bit Manipulation）常用于：判断奇偶、不借助临时变量交换两数、取最低位 1（lowbit = x & -x，树状数组核心）、子集枚举、状态压缩动态规划（如 TSP 旅行商）、权限 / 集合的紧凑表示。常见运算符：与(&)、或(|)、异或(^)、取反(~)、左移(<<)、右移(>>)。面试常问「位运算能解决哪些常见问题」——上面这些就是高频答案。
