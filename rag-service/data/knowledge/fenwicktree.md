...
> **中文导读（面向中文提问）**：树状数组（Fenwick Tree / Binary Indexed Tree，简称 BIT）用于高效处理数组的**单点更新 + 区间求和（前缀和）**，单次操作 O(log n)。它和线段树（Segment Tree）常被一起比较：树状数组代码短、常数小、易写，适合「单点更新 + 区间查询」；线段树更通用，能处理区间最值、区间更新（懒惰传播）等更复杂场景。面试常问「树状数组和线段树有什么区别 / 各自优劣」——本质就是轻量专用 vs 通用全能的权衡。
