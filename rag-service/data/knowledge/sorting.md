...
下面给出常见排序算法在平均 / 最坏情况下的时间复杂度，以及稳定性与是否原地：

| 算法 | 平均时间复杂度 | 最坏时间复杂度 | 空间复杂度 | 稳定性 | 是否原地 |
|------|---------------|---------------|-----------|--------|----------|
| 冒泡排序 Bubble Sort | O(n²) | O(n²) | O(1) | 稳定 | 是 |
| 选择排序 Selection Sort | O(n²) | O(n²) | O(1) | 不稳定 | 是 |
| 插入排序 Insertion Sort | O(n²) | O(n²) | O(1) | 稳定 | 是 |
| 希尔排序 Shell Sort | O(n^1.3) | O(n²) | O(1) | 不稳定 | 是 |
| 堆排序 Heap Sort | O(n log n) | O(n log n) | O(1) | 不稳定 | 是 |
| 归并排序 Merge Sort | O(n log n) | O(n log n) | O(n) | 稳定 | 否 |
| 快速排序 Quick Sort | O(n log n) | O(n²) | O(log n) | 不稳定 | 是 |
| 计数排序 Counting Sort | O(n+k) | O(n+k) | O(k) | 稳定 | 否 |
| 基数排序 Radix Sort | O(d(n+k)) | O(d(n+k)) | O(n+k) | 稳定 | 否 |

记忆要点：比较类排序（冒泡 / 选择 / 插入 / 希尔 / 堆 / 归并 / 快排）的时间下界是 **O(n log n)**；
快速排序在最坏情况下退化为 O(n²)（如输入已近乎有序），工程中常用随机化 pivot 或三数取中来避免；
若要求稳定且 O(n log n)，选**归并排序**；若要求原地且 O(n log n)，选**堆排序**；
若数据范围有限（整数、字符），非比较类排序（计数 / 基数）可做到接近线性 O(n+k)。
