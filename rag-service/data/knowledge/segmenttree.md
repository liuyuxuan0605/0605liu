...
> **中文导读（面向中文提问）**：线段树（Segment Tree）用于在数组上做**区间查询（最值 / 求和）+ 区间更新**，时间复杂度 O(log n)。与树状数组相比：线段树能处理区间最值（RMQ）、区间更新（懒惰传播 lazy update）等更复杂操作，通用性更强；树状数组则更轻量、代码更短。面试常问「线段树怎么做区间查询和单点更新」「树状数组和线段树比有什么优劣」——能答清二者适用场景的取舍即是关键。
