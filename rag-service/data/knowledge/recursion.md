...
> **中文导读（面向中文提问）**：递归（Recursion）与迭代（Iteration）的区别——递归是函数自己调用自己，代码简洁、贴近问题定义，但有栈溢出风险，且若有重叠子问题会指数级重复计算（需靠记忆化 / DP 优化）；迭代用循环实现，通常要手动维护栈或状态，更省空间。必须用递归的场景：问题定义本身就是递归的（树的遍历、分治、回溯、斐波那契）、或递归写法远比迭代清晰时。面试常问「递归和迭代有什么区别，什么时候必须用递归」。
